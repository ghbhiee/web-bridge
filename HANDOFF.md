...
- **测试套件不碰真实 qmd**（`run_tests.sh` 里 `export WEB_BRIDGE_QMD=0`）。
  排序测试只需要确定性的词法打分，而真跑 qmd 会让套件依赖原生二进制、热模型缓存和网络：
  我做变异测试时每条查询卡 30s 超时、后台拉了 1.2GB，还因为连带把套件里的 HTTP 请求
  拖超时而整个崩掉（表现是「0 PASS」，不是某条 FAIL，很容易误读成环境问题）。
  两个关心 qmd 命令本身的测试用 mock `subprocess.run` + 强制 `qmd_available()`，
  照样断言真实命令形状。改完：全套 4.2s，四轮变异零下载。
...
- 索引位置：`INDEX_PATH` + `QMD_CONFIG_DIR` 指向
  ...
  **代价是这套调用姿势在两边各有一份**（2026-08-27 用户确认接受：公开仓库的自足性
  比省掉 ~30 行 subprocess 更重要）。**所以改 `toolsearch.py` 里 qmd 那段之前，
  先去看一眼 `~/.claude/skills/llm-wiki/scripts/kb.py` 的对应实现**——那边是这套知识的
  规范来源，两边今天是同步的，漂移只会从"某一边学到新东西而另一边不知道"开始。
...
### 自我进化：三次重复的门槛几乎从不触发（2026-08-27 实测）

E2E 结论，两半：

**机制通**（mail.tokencv.com，同一段脚本连跑 3 次）：第 2 次给出预告、第 3 次自动沉淀成
`auto-mail-tokencv-com-1ee062`，并通过 `tools_available` 当场告知调用方；沉淀出的工具
按自然语言查询排第一（去掉本页加权仍第一）。参数变化不影响签名（归一化正确）。

**现实不通**（项目中台 jarvis.myhexin.com，两个全新 agent，**完全相同**的任务）：
...
30 天全局：200 段 ad-hoc vs 21 次能力调用；`unogs.com` 46 段里 45 段互不相同，
`mail.myhexin.com` 37 段 0 重复。**distinct ≈ adhoc 是普遍现象。**

**根因：复用的单位选错了。** agent 不重复*脚本*，但会重复*任务*。它摸一个陌生系统会写
十几段——前面全是探路，最后一两段才是交付。规则等的是"重复"，现实给的是"每次都新写"。

**改法（`promote_session_tail`）**：一个站点上的会话结束后（默认 30 分钟无活动，
`config.session_gap_s`），把**最后一段成功的 exec** 沉淀下来。触发点在 `/capabilities`——
新会话开始的那一刻正是上一个会话结束的时刻，且赶在列表构建之前，所以当次就能看到。

**但它是猜测，不是事实**：拿这条规则重放本机日志，17 段尾巴通过了琐碎过滤，其中约 7 段是
真交付，其余是碰巧跑在最后的探针（`{cgo:...}`、`{title,url,body}`、「探一下页面状态」）。
所以尾巴沉淀为 **`provisional`（试用）**：

- **进搜索**（`web_find_tool`）——猜错了就是永远不被排上来，零成本
- **不进目录**（`catalogue()`）——目录能成立的前提是整个库塞得进简报，让猜测进来会花掉
  这份预算，超限后目录路径直接失效
- 试过的两条筛选都不行：`// 说明：` 只有 1/17 有（太严，等于不沉淀）；不加闸门则 40% 精确率
  （太松，库被稀释一倍）

**另外两个已修的具体 bug**：① `looks_trivial` 白名单有 `textContent` 没有 `innerText`，
同一段脚本换个 API 就永远不能沉淀；② 沉淀闸门是 `if kind == "exec"`，脚本一旦存成
Page Tool，后续从面板运行多少次都不触发——有两段各成功 4 次（超过门槛）却没沉淀，
因为 1 次 exec + 3 次 user-script。

**后续实测（同日，发信任务）：复用是真的，但复用的是知识不是代码。**

放一个全新 agent 去 mail.tokencv.com 发信。它先 `wb log --host mail.tokencv.com --code`
翻日志（`journal-read` 埋点抓到，命中 10 条），然后**自己重写了 4 段 JS**——签名一段都没重复。
它从日志里拿走的是「`POST /api/send` + `{to,subject,text,html,attachments}`」这个知识，
不是那段代码。

| 场景 | 查过先例？ | 写了几段脚本 |
|---|---|---|
| jarvis 第 1 轮（无先例） | 否 | 16 |
| jarvis 第 2 轮（有先例） | 只 discover | 10 |
| mail 发信（有先例） | **查了日志** | **4，一次成功** |

**所以「同一段脚本重复 3 次」这个度量从根上measure错了东西**：知情的 agent 依然会自己写码，
签名永远对不上。真正该看的是「有没有查过先例」和「查完之后少写了多少段」。
`usage_stats` 因此新增 `journal_reads` / `consulted_prior`。

顺带一条教训：**agent 的自述不可信，要用埋点核对**。这个 agent 自称「直接照搬参数、没有重新
摸索」，日志显示它 4 段脚本全是新写的——知识确实复用了，但「照搬」是它的美化。

**发信链路已端到端验证**（`/api/send` → 200 → 收件箱 78→79 出现该邮件），
用的是页面自己的登录态，没有任何凭据经手。

**自动沉淀会挑中最没用的那半（2026-08-27，三次发信任务）**

用户从 dsh 连发三封邮件后问「这些 JS 怎么没进能力库」。查下来**进了，但进错了**：

```
沉淀成功: await sendEmail(); await new Promise(r=>setTimeout(r,1500));
          const toast=...; return {modalShown, toast}      ← 跑了 3 次
没沉淀:   openCompose(收件人,主题); compose-body.innerHTML=正文  ← 每次内容不同，各 1 次
```

**一个任务 = [填具体内容] + [不变的提交动作]。重复的永远是后者，有价值的永远是前者。**
按签名重复计数，就会系统性地挑中样板、丢掉实质——而挑中的那半单独运行毫无意义，
它依赖前一段先把草稿填好。

更糟的是它进了目录：标题是一串代码、`params: {}`、★本页，
**任何在这个邮箱页面干活的 agent 都会看到一个"调用即把当前草稿发出去"的工具**。

两道修复：

1. **`acts_outward()`：对外/不可逆动作不自动沉淀。** 命中
   `send|submit|publish|post|buy|pay|transfer` 开头的函数调用、`deleteX(`、
   或 `method: POST/PUT/PATCH/DELETE` 的 fetch。自动沉淀写不出描述也命名不了参数，
   而这两样恰恰是不可逆动作在被按名调用前必须有的。知识不会因此丢失——日志还留着，
   而日志本来就是复用真正发生的地方。
   - 扫**原始代码不能扫 `normalise()` 的结果**：后者把字符串字面量换成哈希，
     `method:"POST"` 里的 POST 就没了。
   - `remove` / `confirm` 故意不在名单里：`el.remove()` 删的是本地节点、
     `confirm()` 是弹窗，都不出页面。带上它们会误伤 reader-mode。
   - 已知盲区：`x-post` 靠 `btn.click()` 发推，那行没有任何"发送"字样，正则识别不了。
     所以下面第 2 条才是结构性的那道闸。
2. **机器描述的工具一律不进目录**（`auto` 或 `provisional` 都排除）。目录成立的前提是
   每一行都是模型扫一眼就能判断的句子，`🤖 await sendEmail();await new Promise(r=>…`
   不是。它们仍在 `search()` 里（实测纯自然语言查询、无本页加权时排第一），
   用 `web_save_capability` 写上真描述就自动进目录。

**已知缺陷**：读 DOM 的能力会**静默返回过期数据**——发完信立刻用沉淀出的能力读收件箱，
返回的还是旧的 78 封，因为页面没刷新。又是一次「失败伪装成成功」，
凡是读 DOM 列表的能力都该考虑先触发页面自身的刷新。
